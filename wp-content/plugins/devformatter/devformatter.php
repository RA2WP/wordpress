...
*/
  ...
  function devfmt_ParseStructure($ACode, $ALang, $AOptions, $AStyle = "", $AAjaxRequest = false){
    ...
    if(!is_feed() && (!$DevFmt_Config['useajaxparse'] || $AAjaxRequest)){
      ...
      for($i = 0; $i < Count($CodeLines); $i++){
        ...
        if($AOptions['lines'] == "lines")
          $lineNum = "<td class=\"" . $lineNumClass . "\" width=\"3%\">" .
    ...
    if($DevFmt_Config['showtools'] && !is_feed() && !IsSet($AOptions['notools'])){
      $Toolbar = '<div class="devcodetools" style="width:100%;">'.
        '<div style="float:left">&nbsp;'.devfmt_getLangTitle($ALang).'&nbsp;|&nbsp;</div>'.
        '<div style="float:left;background-image:url(\''.DEVFMT_URL.'img/devformatter-copy.png\');background-repeat:no-repeat;'.
        ...
        '<div style="float:left">&nbsp;'.str_replace(" ", "&nbsp;", $DevFmt_Config['copyclipboartext']).'&nbsp;|</div>'.
        '<div style="float:left;cursor:pointer" title="DevFormatter Plugin" onclick="devfmt_credits()">?</div>'.
