...
abstract class WPIMCore {
	...
	public static function format_currency( $amount ) {
		...
		if ( $currency_decimal_precision && $currency_decimal_separator ) {
			$decimals = (int) ( ( $amount - (int) $amount ) * pow( 10, $currency_decimal_precision ) );
	...
	protected static function output_errors() {
		...
		if ( self::$plugin_errors ) {
			$content .= '<div class="error"><p>' . implode( '</p><p>', self::$plugin_errors ) . '</p></div>';
		}
	...
	public static function getDisplay( $key ) {
		$display = explode( ',', self::getOption( 'display_' . $key ) );
	...
	public static function dropdown_array( $name, $selected, $array, $class = '' ) {
		$content = '<select name="' . $name . '" class="' . $class . '">' . PHP_EOL;;
	...
	protected static function dropdown_yesno( $name, $selected, $class = '' ) {
		$array = array(
			'0' => self::__( 'No' ),
			'1' => self::__( 'Yes' )
		);
