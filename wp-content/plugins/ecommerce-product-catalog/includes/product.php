...
function create_product() {
	...
	if ( $wp_version < 3.8 ) {
		$reg_settings = array(
			...
			'query_var'				 => $query_var,
			'supports'				 => apply_filters( 'ic_products_type_support', array( 'title', 'thumbnail' ) ),
...
function get_product_query_var() {
	$names		 = get_catalog_names();
	$query_var	 = sanitize_title( ic_strtolower( $names[ 'singular' ] ) );
	$query_var	 = (strpos( $query_var, '%' ) !== false) ? 'product' : $query_var;
...
function add_product_metaboxes() {
	...
	add_meta_box( 'al_product_short_desc', sprintf( __( '%s Short Description', 'ecommerce-product-catalog' ), $names[ 'singular' ] ), 'al_product_short_desc', 'al_product', apply_filters( 'short_desc_box_column', 'normal' ), apply_filters( 'short_desc_box_priority', 'default' ) );
	add_meta_box( 'al_product_desc', sprintf( __( '%s description', 'ecommerce-product-catalog' ), $names[ 'singular' ] ), 'al_product_desc', 'al_product', apply_filters( 'desc_box_column', 'normal' ), apply_filters( 'desc_box_priority', 'default' ) );
	if ( is_ic_price_enabled() || is_ic_sku_enabled() ) {
		add_meta_box( 'al_product_price', sprintf( __( '%s Details', 'ecommerce-product-catalog' ), $names[ 'singular' ] ), 'al_product_price', 'al_product', apply_filters( 'product_price_box_column', 'side' ), apply_filters( 'product_price_box_priority', 'default' ) );
	}
	if ( get_shipping_options_number() > 0 ) {
		add_meta_box( 'al_product_shipping', sprintf( __( '%s Shipping', 'ecommerce-product-catalog' ), $names[ 'singular' ] ), 'al_product_shipping', 'al_product', apply_filters( 'product_shipping_box_column', 'side' ), apply_filters( 'product_shipping_box_priority', 'default' ) );
	}
	if ( product_attributes_number() > 0 ) {
		add_meta_box( 'al_product_attributes', sprintf( __( '%s Attributes', 'ecommerce-product-catalog' ), $names[ 'singular' ] ), 'al_product_attributes', 'al_product', apply_filters( 'product_attributes_box_column', 'normal' ), apply_filters( 'product_attributes_box_priority', 'default' ) );
	}
...
function change_image_box() {
	...
	add_meta_box( 'postimagediv', sprintf( __( '%s Image', 'ecommerce-product-catalog' ), ic_ucfirst( $names[ 'singular' ] ) ), 'post_thumbnail_meta_box', 'al_product', apply_filters( 'product_image_box_column', 'side' ), apply_filters( 'product_image_box_priority', 'high' ) );
}
